...
class MounjaroTracker {
  ...
  setupEventListeners() {
    ...
    if (dailyForm) {
      dailyForm.addEventListener(
        "submit",
        this.handleDailySubmission.bind(this)
      );
    }

    // Medication checkbox toggle
    const medicationCheckboxGroup = document.querySelector(".checkbox-group");
    const medicationCheckboxLabel = document.querySelector(
      "#includeMedication ~ .checkbox-label"
    );
    ...
    if (medicationCheckboxGroup) {
      medicationCheckboxGroup.addEventListener("click", (e) => {
        document.querySelector(".checkbox-group input")?.click();
      });
    }

    if (medicationCheckbox && medicationFields && submitBtn) {
      medicationCheckbox.addEventListener("click", (e) => {
        e.stopPropagation();
      });
      medicationCheckboxLabel.addEventListener("click", (e) => {
        e.stopPropagation();
      });
      medicationCheckbox.addEventListener("change", (e) => {
        if (e.target.checked) {
          medicationFields.style.display = "block";
          submitBtn.innerHTML = `${Utils.getIconHTML(
            "medication",
            18,
            "btn-icon"
          )} Log Check-in + Medication`;
    ...
    const importMethodButtons = document.querySelectorAll(
      "#import .tab-button"
    );
  ...
  handleDailySubmission(e) {
    ...
    try {
      const includeMedication =
        document.getElementById("includeMedication").checked;
      ...
      if (includeMedication) {
        ...
        const medicationNotes =
          document.getElementById("medicationNotes").value || null;

        if (!dose) {
          Utils.showMessage(
            "Please select a dose when logging medication",
            "error",
            "dailyMessages"
          );
          return;
      ...
      Utils.showMessage(
        `${message} Total entries: ${this.dataManager.getEntries().length}`,
        "success",
        "dailyMessages"
      );
      this.updateUI();
  ...
  updateDaysSinceLastDose() {
    ...
    if (lastMedication) {
      ...
      daysSinceDiv.innerHTML = `<div class="days-since">
                ${daysSince} day${
        daysSince !== 1 ? "s" : ""
      } since last Mounjaro dose (${lastMedication.dose})
            </div>`;
    } else {
      daysSinceDiv.innerHTML = `<div class="days-since">
  ...
  displayEntries() {
    ...
    if (entries.length === 0) {
      entriesList.innerHTML =
        '<div class="no-data">No entries yet. Start by logging your daily check-in or medication dose!</div>';
      return;
    }
        ...
        if (entry.type === "medication") {
          return `
                    <div class="entry-card medication">
                        <div class="entry-header">
                            <span class="entry-type medication">${Utils.getIconHTML(
                              "medication",
                              30,
                              "entry-icon"
                            )} Medication</span>
                            <button class="delete-btn" onclick="app.deleteEntry(${
                              entry.id
                            })">${Utils.getIconHTML(
            "delete",
            30,
          ...
          return `
                    <div class="entry-card daily">
                        <div class="entry-header">
                            <span class="entry-type">${Utils.getIconHTML(
                              "daily",
                              30,
                              "entry-icon"
                            )} Daily Check-in</span>
  ...
  displayStats() {
    ...
    if (weightEntries.length >= 3) {
      ...
      if (trendData) {
        const totalChange =
          trendData.slope * trendData.xValues[trendData.xValues.length - 1];
        const changeText = `${totalChange > 0 ? "+" : ""}${totalChange.toFixed(
          1
        )}kg`;
    ...
    statsContent.innerHTML = `
            ...
            <button class="btn export-btn" onclick="app.exportData()">${Utils.getIconHTML(
              "export",
              30,
              "btn-icon"
            )} Export Data</button>
